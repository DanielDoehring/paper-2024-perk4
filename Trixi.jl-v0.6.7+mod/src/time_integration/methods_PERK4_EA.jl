...
mutable struct PERK4_EA{StageCallbacks}
    ...
    function PERK4_EA(NumStages_::Int, BasePathMonCoeffs_::AbstractString,
                   stage_callbacks = ())
        newPERK4_EA = new{typeof(stage_callbacks)}(NumStages_, stage_callbacks)

        newPERK4_EA.AMatrices, newPERK4_EA.AMatrix, newPERK4_EA.c = ComputePERK4_ButcherTableau(NumStages_,
                                                                                       BasePathMonCoeffs_)
...
mutable struct PERK4_EA_Integrator{RealT <: Real, uType, Params, Sol, F, Alg,
...
function Base.getproperty(integrator::PERK4_EA_Integrator, field::Symbol)
    if field === :stats
...
function init(ode::ODEProblem, alg::PERK4_EA;
    ...
    integrator = PERK4_EA_Integrator(u0, du, u_tmp, t0, dt, zero(dt), iter, ode.p,
                                  (prob = ode,), ode.f, alg,
...
function solve(ode::ODEProblem, alg::PERK4_EA;
               dt, callback = nothing, kwargs...)
...
function solve_steps!(integrator::PERK4_EA_Integrator)
...
function k1!(integrator::PERK4_EA_Integrator, p, c)
...
function last_three_stages!(integrator::PERK4_EA_Integrator, alg, p)
    for stage in 1:2
        @threaded for u_ind in eachindex(integrator.u)
...
function step!(integrator::PERK4_EA_Integrator)
    @unpack prob = integrator.sol
...
get_du(integrator::PERK4_EA_Integrator) = integrator.du
get_tmp_cache(integrator::PERK4_EA_Integrator) = (integrator.u_tmp,)

# some algorithms from DiffEq like FSAL-ones need to be informed when a callback has modified u
u_modified!(integrator::PERK4_EA_Integrator, ::Bool) = false

# used by adaptive timestepping algorithms in DiffEq
function set_proposed_dt!(integrator::PERK4_EA_Integrator, dt)
    integrator.dt = dt
end

function get_proposed_dt(integrator::PERK4_EA_Integrator)
    return integrator.dt
end

# stop the time integration
function terminate!(integrator::PERK4_EA_Integrator)
    integrator.finalstep = true
    empty!(integrator.opts.tstops)
end

# used for AMR (Adaptive Mesh Refinement)
function Base.resize!(integrator::PERK4_EA_Integrator, new_size)
